...
const REVIEW_ANSWER_OVERRIDES = {
  "Психодиагностика#3": 1,
  "Психодиагностика#21": 2,
};

const TOPIC_ID_PREFIXES = {
  "Общая психология": "obshaya-psihologiya",
  "Возрастная психология": "vozrastnaya-psihologiya",
  "Социальная психология": "socialnaya-psihologiya",
  "Теории и методы психотерапии": "teorii-i-metody-psihoterapii",
  "Введение в патопсихологию": "vvedenie-v-patopsihologiyu",
  "Психодиагностика": "psihodiagnostika",
};
...
async function initialize() {
  ...
  try {
    ...
    const data = normalizeQuizData(rawData);
    state.data = data;
    state.topics = data.topics.map((entry) => entry.title);
    state.questions = data.questions.map((question) => applyReviewOverride(question));
...
function normalizeQuizData(rawData) {
  ...
  const normalizedQuestions = rawQuestions
    ...
    .filter((question) => REQUIRED_TOPICS.includes(question.topic));
  ...
  const topics = REQUIRED_TOPICS
    .map((title) => ({
      title,
      slug: TOPIC_ID_PREFIXES[title],
      count: topicCounts.get(title) || 0,
    }))
    .filter((topic) => topic.count > 0);

  return {
    meta: {
      title: normalizePossibleMojibake(rawData?.meta?.title || "Викторина"),
...
function normalizeQuestion(question) {
  ...
  const prefix = TOPIC_ID_PREFIXES[topic] || "topic";

  return {
    id: `${prefix}-${number}`,
    topic,
    number,
    question: normalizePossibleMojibake(question?.question || ""),
    options: Array.isArray(question?.options)
      ? question.options.map((option) => normalizePossibleMojibake(option))
      : [],
...
function normalizeTopic(topic) {
  const normalized = normalizePossibleMojibake(topic || "").trim();
  const exactMatch = REQUIRED_TOPICS.find((entry) => entry === normalized);
  if (exactMatch) {
    return exactMatch;
  }

  const normalizedKey = slugifyTopic(normalized);
  return REQUIRED_TOPICS.find((entry) => slugifyTopic(entry) === normalizedKey) || normalized;
}

function normalizePossibleMojibake(value) {
  const stringValue = String(value ?? "");
  if (!/[РСÐÑ]/.test(stringValue)) {
    return stringValue;
  }
...
function applyReviewOverride(question) {
  const override = REVIEW_ANSWER_OVERRIDES[`${question.topic}#${question.number}`];
